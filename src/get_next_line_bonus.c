...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/06/16 12:26:19 by ppour-ba          #+#    #+#             */
/*   Updated: 2023/06/16 12:27:08 by ppour-ba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	stat[fd] = first_line(fd, stat[fd]);
	if (!stat[fd] || stat[fd][0] == '\0')
		return (NULL);
	new_line = ft_new_line(stat[fd]);
